...
public class ConsoleCommandCharTemp extends CommandBaseConsole {
    ...
    public static void reApply(Player requester) {
        ...
        if (entry == null) {
            SneakyCharacterManagerPaper.logger().warn("Attempted to reapply a temp character for player {} that isn't on a temp character [UUID: {}].", requester.getName(), playerUUID);
            return;
        }
